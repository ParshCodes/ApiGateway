...
| Proxy layer | `http-proxy` stream passthrough | No body buffering — correct for large payloads and streaming |
...
The script runs three scenario groups and prints formatted results tables.
Follow the in-terminal prompts to reconfigure the gateway between groups.

### Group 1 — 3 backends · no rate limiting *(baseline)*

| Scenario | RPS | p50 ms | p97.5 ms | p99 ms | Errors | Timeouts |
|----------|-----|--------|----------|--------|--------|----------|
| 50 RPS   |     |        |          |        |        |          |
| 200 RPS  |     |        |          |        |        |          |
| 500 RPS  |     |        |          |        |        |          |
...
### Group 2 — 3 backends · rate limiting ON *(100 req / 60 s)*
...
### Group 3 — Scaling impact: 1 backend vs 3 backends

| Load | Backends | RPS | p50 ms | p97.5 ms |
|------|----------|-----|--------|----------|
| 200 RPS | 1 | | | |
| 200 RPS | 3 | | | |
| 500 RPS | 1 | | | |
...
> **After running benchmarks**, paste your numbers into the tables above — the delta between groups tells the story of what this gateway actually buys you.
...
| Proxy | `http-proxy` | Stream passthrough — no response buffering |
